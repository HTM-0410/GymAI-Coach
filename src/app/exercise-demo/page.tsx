    ...
    <div className="mx-auto max-w-6xl px-4 pb-24 pt-6">
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_310px]">
        <div className="space-y-5">
          <Panel className="p-3"><div className="relative h-56 overflow-hidden rounded-md bg-slate-900 sm:h-64"><Image src="/exercises/demo/bench-press-main.jpg" alt="Barbell bench press" fill priority className="object-cover"/><div className="absolute left-3 top-3 flex rounded-lg bg-black/80 p-1 text-[10px] font-bold text-white"><span className="rounded bg-accent px-2 py-1">HÌNH ẢNH</span><span className="px-2 py-1 text-slate-300">VIDEO</span><span className="px-2 py-1 text-slate-300">CƠ HOẠT ĐỘNG</span></div><div className="absolute inset-x-0 bottom-0 bg-black/75 px-4 py-3 text-sm font-medium text-white">Tư thế chuẩn: bả vai cố định, chân đạp chắc sàn</div></div><div className="mt-3 flex gap-2">{['main','side','top'].map((view, index) => <div key={view} className={`relative h-12 w-20 overflow-hidden rounded border-2 ${index === 0 ? 'border-accent' : 'border-chassis-lo'}`}><Image src={`/exercises/demo/bench-press-${view}.jpg`} alt={`Góc nhìn ${index + 1}`} fill className="object-cover"/></div>)}</div></Panel>
