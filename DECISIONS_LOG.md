...
| Rationale | spec §49 explicitly: "Không tách FastAPI ? MVP." FastAPI adds infrastructure complexity (separate service, deployment pipeline) without MVP benefit. AI calls (Gemini, fal.ai, Byesu) are all HTTP-based ? no performance reason for Python. |
...
| Rationale | Human answered OQ-5: "Không h? tr? offline MVP". Offline adds significant complexity (conflict resolution, sync queue, IndexedDB management). Most gyms have connectivity. Phase 3 PWA feature will address this properly. |
...
| Rationale | Human answered OQ-6: "Gi? vô h?n". Auto-cleanup adds cron jobs and risk of data loss. Storage cost manageable for MVP scale. Manual admin cleanup sufficient. |
...
| Rationale | Human answered OQ-1: "Thêm optional checkbox vai/l?ng/??u g?i". Sufficient for MVP ? AI uses `exercise_alternatives` table to suggest substitutions when injury detected. Full medical database is overkill. Checkboxes are optional and do not block onboarding. |
...
| Rationale | spec §26 explicitly: "?ây là màn hình quan tr?ng nh?t trên mobile." Users log sets at the gym ? sweaty, one hand, between sets, limited time. Every design decision serves speed and clarity. Desktop gets full analytics; mobile gets fast logging. |
...
**Decision:** Add `.eslintrc.json` v?i `extends: "next/core-web-vitals"` ?? có lint gate.
**Reason:** Repo thi?u ESLint config ? `npm run lint` interactive prompt ? không verify ???c. TIP-021C follow-up sau verify TIP-021B.
**Impact:** L?n ??u ch?y lint phát hi?n 9 pre-existing errors (`react/no-unescaped-entities` ? 3 form files) + 1 warning (`@next/next/no-img-element` ? blob URL). T?t c? fix b?ng inline `eslint-disable-next-line` (an toàn cho unicode apostrophe + blob URL không th? dùng `next/image`).
**Reversal Risk:** None ? config chu?n Next.js docs.

## D-033: Seed script type fix (2026-08-19, TIP-021D)
**Decision:** S?a 1 dòng trong `scripts/seed-exercises-data.ts:553` ? ??i `slug: 'Nâng gót Smith ??ng'` thành `name_vi: 'Nâng gót Smith ??ng'` (key b? sai do copy-paste).
**Reason:** Build type-check fail vì duplicate key `slug` + missing required `name_vi`.
**Impact:** Build pass. Bài `Smith Machine Standing Calf Raise` gi? có `name_vi: 'Nâng gót Smith ??ng'` ? ?úng ngh?a. Có th? seed DB OK.
**Reversal Risk:** None.

## D-034: Root layout cleanup (2026-08-19, TIP-021E inline)
**Decision:** B? duplicate import `Metadata, Viewport` ? `src/app/layout.tsx` line 2 + b? `#region agent log` instrumentation fetch còn sót.
**Reason:** Pre-existing duplicate import + debug instrumentation break `next build` type-check. Không thu?c scope TIP-021B nh?ng user yêu c?u "FIX NGAY" nên x? lý inline.
**Impact:** Build pass hoàn toàn. Không thay ??i runtime behavior.
**Reversal Risk:** None.
**Decision:** Move staging `/exercises-test/*` ? `(app)/exercises/*`; gi? `/exercise-demo` làm design reference v?i banner `@deprecated`.
**Reason:** TIP-021A yêu c?u route `/exercises` chu?n; staging `/exercises-test` ch? là intermediate; demo `/exercise-demo` c?n gi? ?? so sánh visual v?i JSON-driven detail page.
**Impact:** Nav ?ã tr? `/exercises` s? b?t ??u resolve; smoke test c?n user onboarded (auth gate trong `(app)/layout.tsx`).

## D-030: Design reference policy (2026-08-18)
**Decision:** Pages trong `src/app/exercise-demo/` không bao gi? xóa h?n khi còn component ???c import t? route khác. Khi mu?n xóa ph?i extract component ra `src/components/exercise/` tr??c.
**Reason:** Tránh m? côi import (`VideoGuide`, `PerformanceChart` ?ang ???c `exercises/[slug]/page.tsx` import tr?c ti?p).
**Impact:** Maintenance cost: 4 file demo v?n t?n t?i, nh?ng ch? `page.tsx` thêm banner; 3 component file gi? nguyên.

## D-031: Placeholder image policy (2026-08-18)
**Decision:** T?o `public/exercises/demo/PLACEHOLDER.svg` dùng chung cho 6/8 bài ch?a có ?nh th?t (barbell-row, deadlift, hip-thrust, overhead-press, pull-up, romanian-deadlift).
**Reason:** Phase này không ??ng AI image generation (TIP-007A Phase 2). Tránh broken image ?nh h??ng UX verify.
**Impact:** Placeholder s? ???c thay th? b?ng Gemini Search grounding URLs trong TIP-007A. Schema JSON không c?n ??i ? ch? path `gallery.main` + `gallery.views[].src` thay ??i khi có URL th?t.
